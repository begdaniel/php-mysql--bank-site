...
function has_unique_page_menu_name($page) {
  if(!isset($page['id'])) {
    ...
    while($db_page = mysqli_fetch_assoc($page_set)){
      if (strtolower($menu_name) === strtolower($db_page['menu_name'])) {
        mysqli_free_result($page_set);
        return false;
      }
    }
...
/*
function has_unique_admin_username($admin) {
  if(!isset($admin['id'])) {
    $username = $admin['username'];
    $admin_set = find_all_admins();
    while($db_admin = mysqli_fetch_assoc($admin_set)){
      if (strtolower($username) === strtolower($db_admin['username'])) {
        mysqli_free_result($admin_set);
        return false;
      }
    }
